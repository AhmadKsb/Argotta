...
public class FinalStepActivity extends BaseActivity implements RegistrationContract.View,ProfileInfoContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(!isEmailValid(email) && username.matches("") && password.matches("")){  //all fields are empty.
                    showError("Please fill all the fields.");
                    return;
                }
                if(isEmailValid(email) && username.matches("") && password.matches("")){   //user and pass are empty.
                    showError("Username and password fields can not be empty.");
                }
                if(isEmailValid(email) && !username.matches("")
                        && username.matches("[a-zA-Z]+") && password.matches("")){  //pass is empty.
                    showError("Password field can not be empty.");
                    return;
                }
                if(isEmailValid(email) && !username.matches("") && !username.matches("[a-zA-Z0-9]+")
                        && !password.matches("")){
                ...
                if (!isEmailValid(email) && username.matches("") && !password.matches("")) {
                ...
                if(checkbox1.isChecked() && isEmailValid(email) && !username.matches("")
    ...
    public void onRegistrationSuccess() {
        onRequestEnd();
        intent = new Intent(FinalStepActivity.this, WelcomeActivity.class);
        ...
        model.email=bundle.getString("email");
        model.display_name=bundle.getString("username");
        //model.country=bundle.getString("country");
        model.language=bundle.getString("english_language");
    ...
    {
        getPrefs().edit().putString("language", intent.getStringExtra("language"))
                .putString("english_language", intent.getStringExtra("english_language"))
                .apply();
